...
def get_songs_by_artist(artist:str, page:int = 0, size:int=10):
    return song.songs_by_artist(artist)[page*size:(page+1)*size]

@app.get("/year/{year}")
def get_songs_by_year(year:int, page:int = 0, size:int=10):
    return song.songs_by_year(year)[page*size:(page+1)*size]

@app.get("/genre/{genre}")
def get_artist_by_genre(genre:str, page:int = 0,size:int=10):
